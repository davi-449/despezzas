...
class GerarPDFVenda(CustomView):
    ...
    def gerar_pdf(self, title, venda, user_id):
        ...
        venda_report = VendaReport(venda)
        ...
        if hasattr(venda_report.topo_pagina, 'inserir_data_emissao'):
        ...
        pdf = canvas.Canvas(venda_pdf, pagesize=A4)
        venda_report.drawOn(pdf)
        pdf.save()
        resp.write(venda_pdf.getvalue())
...
class GerarPDFPedidoVenda(GerarPDFVenda):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response['Content-Disposition'] = f'attachment; filename="{self.get_filename(venda)}"'
        ...
        elements.append(Paragraph(f"{self.get_title(venda)}: {venda.numero}", styles['Title']))
        elements.append(Paragraph(f"Cliente: {venda.cliente.nome}", styles['Normal']))
        if isinstance(venda, OrcamentoVenda):
            elements.append(Paragraph(f"Validade: {venda.data_vencimento.strftime('%d/%m/%Y')}", styles['Normal']))
        elif isinstance(venda, PedidoVenda):
            elements.append(Paragraph(f"Data de Entrega: {venda.data_entrega.strftime('%d/%m/%Y')}", styles['Normal']))
        ...
        elements.append(Paragraph(f"Total: R$ {venda.total:.2f}", styles['Normal']))
        elements.append(Paragraph("Obrigado pela sua compra!", styles['Normal']))
    ...
    def get_venda(self, venda_id):
        return PedidoVenda.objects.filter(pk=venda_id).first()

    def get_title(self, venda):
        return "Pedido"

    def get_filename(self, venda):
        return f"pedido_{venda.numero}.pdf"
    
class GerarPDFOrcamentoVenda(GerarPDFVenda):
    
    def get(self, request, *args, **kwargs):
        # Obter o orçamento a partir da URL ou de outra fonte (como parâmetros ou banco de dados)
        orcamento_id = kwargs.get('pk', None)
        orcamento = OrcamentoVenda.objects.filter(pk=orcamento_id).first()
        ...
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="orcamento_venda_{orcamento.id}.pdf"'
        ...
        # Cabeçalho do PDF
        elements.append(Paragraph(f"Orçamento: {orcamento.id}", styles['Title']))
        elements.append(Paragraph(f"Cliente: {orcamento.cliente.nome_razao_social}", styles['Normal']))
        elements.append(Paragraph(f"Validade: {orcamento.data_vencimento.strftime('%d/%m/%Y')}", styles['Normal']))
        elements.append(Paragraph(f"Data: {orcamento.data_emissao.strftime('%d/%m/%Y')}", styles['Normal']))

        # Tabela com os itens do orçamento
        data = [
            ["Produto", "Quantidade", "Preço Unitário", "Total"]
        ]

        # Iterar sobre os itens do orçamento e adicionar à tabela
        for item in orcamento.itens_venda.all():
            data.append([
