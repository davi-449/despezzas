...
class GerarPDFCompra(CustomView):
      ...
      def gerar_pdf(self, title, compra, user_id):
        ...
        # Buffer para gerar o PDF
        compra_pdf = io.BytesIO()

        # Instancia o relatório
        compra_report = CompraReport(compra_id=compra.id)

        # Define o título do relatório
        compra_report.title = title

        styles = getSampleStyleSheet()
        compra_report.band_page_header = [
            [Paragraph(u'<b>Relatório de Compra</b>', styles["Title"])],
            [Spacer(1, 1 * inch)],
        ]
        

        # Define o topo da página
        compra_report.topo_pagina = compra_report.band_page_header

        # Configura o cabeçalho
        compra_report.band_page_header = compra_report.topo_pagina

        # Constrói o PDF com o rodapé
        doc = SimpleDocTemplate(compra_pdf, pagesize=A4)

        # Adiciona o rodapé em cada página
        def add_page_footer(canvas, doc):
            compra_report.page_footer(canvas, doc)

        # Construir o PDF com o rodapé sendo adicionado a cada página
        doc.build(compra_report.elements, onLaterPages=add_page_footer)
        ...
        try:
            # Busca informações da empresa e logo com base no usuário
            ...
            # Se o logo for diferente do padrão, insere no relatório
            if flogo != 'imagens/logo.png':
                if hasattr(compra_report.topo_pagina[0][0], 'inserir_logo'):
                    compra_report.topo_pagina[0][0].inserir_logo(logo_path)

            # Insere informações da empresa no rodapé
            from .report_compras import pagefooter
            compra_report.band_page_footer = pagefooter(compra_report, doc=compra_report.doc)
            ...
            if m_empresa.m_empresa.endereco_padrao:
                try:
                    ...
                    pass
            if m_empresa.m_empresa.telefone_padrao and hasattr(compra_report.band_page_footer, 'inserir_telefone_empresa'):
            ...
            pass  # Se ocorrer um erro ao buscar as informações, simplesmente segue sem o logo e informações adicionais

        # Insere a data de emissão no topo do relatório
        if hasattr(compra_report.topo_pagina[0], 'inserir_data_emissao'):
            try:
                compra_report.topo_pagina[0].inserir_data_emissao(compra.data_emissao)
            except AttributeError:
                pass

        # Verifica o tipo de compra (Orçamento ou Pedido) para inserir a data correta
        if isinstance(compra, OrcamentoCompra):
            if hasattr(compra_report.topo_pagina[0], 'inserir_data_validade'):
                compra_report.topo_pagina[0].inserir_data_validade(compra.data_vencimento)
        elif isinstance(compra, PedidoCompra):
            if hasattr(compra_report.topo_pagina[0], 'inserir_data_entrega'):
                compra_report.topo_pagina[0].inserir_data_entrega(compra.data_entrega)

        # Configura o cabeçalho
        compra_report.band_page_header = compra_report.topo_pagina

        # Insere informações do fornecedor, verificando se é pessoa física (PF) ou jurídica (PJ)
        ...
        # Insere informações de endereço, telefone e email do fornecedor
        ...
        # Adiciona os dados do fornecedor ao cabeçalho
        compra_report.band_page_header.append(compra_report.dados_fornecedor)

        # Configura a altura da banda dos produtos com base nos itens da compra
        if hasattr(compra_report, 'banda_produtos'):
            compra_report.banda_produtos.height = len(ItensCompra.objects.filter(compra_id=compra.id)) * 0.5

        # Adiciona a banda de produtos
        compra_report.band_page_header.append(compra_report.dados_produtos.table)

        # Adiciona a seção de totais de venda
        try:
            compra_report.band_page_header.append(compra_report.totais_venda)
        except AttributeError:
            pass

        # Se houver condições de pagamento, adiciona essa seção ao relatório
        if hasattr(compra_report, 'banda_pagamento') and compra.cond_pagamento:
        ...
        # Constrói o PDF
        doc = SimpleDocTemplate(compra_pdf, pagesize=A4)
        doc.build(compra_report.elements)
...
class GerarPDFOrcamentoCompra(GerarPDFCompra):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Busca o objeto do orçamento de compra
...
class GerarPDFPedidoCompra(GerarPDFCompra):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Busca o objeto do pedido de compra
